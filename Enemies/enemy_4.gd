...
func behavior(delta: float) -> void:
	if abs(body.get_angle_to(player.global_position)) >= 0.17:
		...
		if global_position.distance_to(player.global_position) > Values.player_range * randf_range(1.5, 2) and shooting:
			shooting = false
			shoot_timer.stop()
		elif not shooting:
			shooting = true
			shoot_timer.start()
		if global_position.distance_to(player.global_position) <= Values.player_range * randf_range(1.3, 1.4):
